...
- (void)_initialize
{
    ...
    NSString *localizationTouchIDUse;
    NSString *localizationTouchIDEnable;
    NSString *localizationDone;
    NSString *language = [[[NSBundle mainBundle] preferredLocalizations] objectAtIndex:0];
    if ([language isEqualToString:@"ru"]) {
        localizationTouchIDUse = @"Хотите ли Вы использовать Touch ID для аутификации?";
        localizationTouchIDEnable = @"Использовать Touch ID";
        localizationDone = @"Готово";
    } else {
        localizationTouchIDUse = @"Do you want to use Touch ID for authentication?";
        localizationTouchIDEnable = @"Enable Touch ID";
        localizationDone = @"Done";
    }
    ...
    self.messageLabel.text = NSLocalizedStringFromTable(localizationTouchIDUse, @"BKPasscodeView", @"Touch ID를 사용하시겠습니까?");
    self.messageLabel.font = [UIFont preferredFontForTextStyle:UIFontTextStyleHeadline];
    [self addSubview:self.messageLabel];
    
    self.titleLabel = [[UILabel alloc] init];
    self.titleLabel.text = NSLocalizedStringFromTable(localizationTouchIDEnable, @"BKPasscodeView", @"Touch ID 사용");
    ...
    [self.doneButton setTitle:NSLocalizedStringFromTable(localizationDone, @"BKPasscodeView", @"확인") forState:UIControlStateNormal];
